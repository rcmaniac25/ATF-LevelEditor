...
namespace Sce.Atf.Controls.Adaptable.Graphs
{
    ...
    public class D2dCircuitRenderer<TElement, TWire, TPin> : AtfDrawingGraphRenderer<TElement, TWire, TPin>, IDisposable
        ...
        where TPin : class, ICircuitPin
    {
        ...
        public D2dCircuitRenderer(AtfDrawingDiagramTheme theme, IDocumentRegistry documentRegistry = null)
        {
            m_theme = theme;
            ...
            EdgeThickness = 2.0f;
            m_subGraphPinBrush = AtfDrawingFactory.CreateSolidBrush(Color.SandyBrown);
        ...
        public AtfDrawingDiagramTheme Theme
        {
            get { return m_theme; }
            set
            {
        ...
        /// <summary>
        /// Gets or sets IAtfBrush for subgraph pins</summary>
        ...
        public override void Draw(TElement element, DiagramDrawingStyle style, IAtfGraphics g)
        {
        ...
        /// <param name="style">Diagram drawing style</param>
        /// <param name="g">Graphics object</param>
        public override void Draw(TWire edge, DiagramDrawingStyle style, IAtfGraphics g)
        {
            ...
            IAtfBrush pen = (style == DiagramDrawingStyle.Normal) ? GetPen(inputPin) : m_theme.GetOutLineBrush(style);
        ...
        private void DrawEdgeUsingStyleInfo(IEdgeStyleProvider edgeStyleProvider, IAtfBrush pen, IAtfGraphics g)
        {
        ...
        public Point GetGroupPinPosition(ICircuitGroupType<TElement, TWire, TPin> group, ICircuitGroupPin<TElement> groupPin, bool inputSide, IAtfGraphics g)
        {
        ...
        public override void DrawPartialEdge(
            ...
            IAtfGraphics g)
        {
            IAtfBrush pen = outputPin != null ? GetPen(outputPin) : GetPen(inputPin);
        ...
        public override RectangleF GetBounds(TElement element, IAtfGraphics g)
        {
        ...
        public override GraphHitRecord<TElement, TWire, TPin> Pick(
            IGraph<TElement, TWire, TPin> graph, TWire priorityEdge, PointF p, IAtfGraphics g)
        {
        ...
        /// <returns>Hit record containing node and/or edge hit by the given point</returns>
        public override GraphHitRecord<TElement, TWire, TPin> Pick(
            IEnumerable<TElement> nodes, IEnumerable<TWire> edges, TWire priorityEdge, PointF p, IAtfGraphics g)
        {
        ...
        protected virtual bool PickEdge(TWire edge, PointF p, IAtfGraphics g, float xOffset = 0, float yOffset = 0)
        {
        ...
        private void Draw(TElement element, IAtfGraphics g, bool outline)
        {
            ElementTypeInfo info = GetElementTypeInfo(element, g);
            ...
            var gradientBrush = fillBrush as IAtfLinearGradientBrush;
            ...
            if (useRoundedRect)
            {
                ...
                if (TitleBackgroundFilled)
                {
                    ...
                    // We don't want the lower part of the title rect rounded. Some graphics systems (like D2d) do not support half-rounded rect, fill again
            ...
            if (image != null)
            {
                ...
                if (bitMap != null)
                    g.DrawBitmap(bitMap, new RectangleF(p.X + info.Interior.X, p.Y + info.Interior.Y, info.Interior.Width, info.Interior.Height), 1, AtfDrawingBitmapInterpolationMode.Linear);
            ...
            if (drawText)
            {
                ...
                if (!string.IsNullOrEmpty(title))
                {
                    ...
                    m_theme.TextFormat.TextAlignment = AtfDrawingTextAlignment.Center;
                    m_theme.TextFormat.ParagraphAlignment = AtfDrawingParagraphAlignment.Center;
        ...
        /// <param name="g">IAtfGraphics object</param>
        protected void DrawExpandedGroup(TElement element, IAtfGraphics g)
        {
        ...
        protected virtual bool Pick(TElement element, IAtfGraphics g, PointF p)
        {
        ...
        private void DrawExpandedGroupPins(TElement element, IAtfGraphics g)
        {
            var group = element.Cast<ICircuitGroupType<TElement, TWire, TPin>>();
        ...
        private void DrawGhost(TElement element, IAtfGraphics g)
        {
            ElementTypeInfo info = GetElementTypeInfo(element, g);
            Point p = element.Bounds.Location;
            p.Offset(WorldOffset(m_graphPath));
        ...
        private void DrawOutline(TElement element, IAtfBrush pen, IAtfGraphics g)
        {
            ElementTypeInfo info = GetElementTypeInfo(element, g);
            Point p = element.Bounds.Location;
            RectangleF bounds = new RectangleF(p, info.Size);
        ...
        private TPin PickInput(TElement element, IAtfGraphics g, PointF p)
        {
            ElementTypeInfo info = GetElementTypeInfo(element, g);
        ...
        private TPin PickOutput(TElement element, IAtfGraphics g, PointF p)
        {
        ...
        private GraphHitRecord<TElement, TWire, TPin> PickExpander(ICircuitGroupType<TElement, TWire, TPin> pickedElement, PointF p, IAtfGraphics g)
        {
        ...
        private Pair<IEnumerable<TElement>, object> PickSubItem(ICircuitGroupType<TElement, TWire, TPin> pickedElement, PointF p, IAtfGraphics g,
        ...
        private IEnumerable<TElement> PickSubItems(ICircuitGroupType<TElement, TWire, TPin> pickedElement, RectangleF rect, IAtfGraphics g)
        {
            if (!pickedElement.Expanded)
        ...
        /// <param name="g">Graphics object</param>
        /// <param name="pen">IAtfBrush pen</param>
        ...
        protected void DrawWire(IAtfGraphics g, IAtfBrush pen, float x1, float y1, float x2, float y2, float strokeWidth, IAtfStrokeStyle strokeStyle)
        {
        ...
        //private void DrawLine(IAtfGraphics g, IAtfBrush pen, float x1, float y1, float x2, float y2, float strokeWidth, D2dStrokeStyle strokeStyle)
        //{
        //    float thickness = strokeWidth == 0 ? EdgeThickness : strokeWidth;
        //    g.DrawLine(
        //            new PointF(x1, y1),
        ...
        protected virtual IAtfBrush GetPen(TPin pin)
        {
            IAtfBrush pen = m_theme.GetCustomBrush(pin.TypeName);
            if (pen == null)
        ...
        private RectangleF GetWireBounds(TWire wire, IAtfGraphics g)
        {
                ...
                foreach (var edgeInfo in wire.Cast<IEdgeStyleProvider>().GetData(this, WorldOffset(m_graphPath), g))
                {
                    ...
                    else if (edgeInfo.ShapeType == EdgeStyleData.EdgeShape.Line)
                    {
                        ...
                        if (line != null)
                        {
                            if (firstTime)
                            {
                                result = AtfDrawingUtil.MakeRectangle(line[0], line[1]);
                                firstTime = false;
                            }
                            else
                                result = RectangleF.Union(result, AtfDrawingUtil.MakeRectangle(line[0], line[1]));
                        }
            ...
            else
            {
                ...
                result = AtfDrawingUtil.MakeRectangle(new PointF(x1, y1), new PointF(x2, y2));
            }
        ...
        private AtfDrawingRoundedRect m_elementBody = new AtfDrawingRoundedRect();
        private AtfDrawingDiagramTheme m_theme;
